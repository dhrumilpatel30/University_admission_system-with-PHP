...
$sql = "SELECT * FROM `applicatiions` WHERE student_id='" . $result['student_id'] . "'";
$result1 = mysqli_query($link, $sql);
$aplications=mysqli_num_rows($result1);
...
else
{
        ...
        if (mysqli_query($link,$sql))
        {
         ...
         $sql ="UPDATE `courses` SET `filled_seat`= filled_seat+1 ,`requests`= requests- $aplications  WHERE course='".$course."'";
         mysqli_query($link,$sql);
